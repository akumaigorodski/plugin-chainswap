...
class WireSpec extends AnyFunSuite {
  test("Correctly process inner messages") {
    assert(decode(toUnknownMessage(SwapInRequest)) === SwapInRequest)
    ...
    assert(decode(toUnknownMessage(inner1)) === inner1)
    ...
    assert(decode(toUnknownMessage(inner2)) === inner2)

    val sor = SwapOutResponse(100000000000L.sat, 90000.sat, "00" * 512)
    assert(decode(toUnknownMessage(sor)) === sor)

    val swd = SwapInWithdrawDenied(paymentRequest = "00" * 512, reason = "00" * 64)
    assert(decode(toUnknownMessage(swd)) === swd)

    val sod = SwapOutDenied(btcAddress = "00" * 32, reason = "00" * 512)
    assert(decode(toUnknownMessage(sod)) === sod)
  }
}
